...
| [1.7](https://github.com/intelbetteramd-sys/pixroost/issues/24) | Спайк S-03: передача на ПК | Ktor Server на ПК, самоподписанный сертификат, пиннинг на телефоне, докачка после обрыва; Android и iPhone, на iPhone — фоновая загрузка `URLSession` | ≥ 20 МБ/с по Wi-Fi, обрыв посередине не ломает файл | 16 |
...
| [1.10](https://github.com/intelbetteramd-sys/pixroost/issues/27) | Спайк S-07: установщик Windows | `.msi` и MSIX, трей, автозапуск; правило брандмауэра для частных и общедоступных сетей: `windows.firewallRules` в MSIX, действие установщика `.msi` | ставится и запускается на чистой Windows | 8 |
...
| 4.2 | Сервер приёма на ПК | Ktor Server, TLS-сертификат из ключа ПК; решить, закрывать ли порт в сетях, где не было сопряжения | — | 10 |
...
| 4.4 | Поиск ПК | mDNS на обеих сторонах, подсказка про VPN, статус «в сети / не в сети» | `A-Devices` | 6 |
