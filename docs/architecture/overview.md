...
- **Серверов Homeroll нет** (в v1.0). Почему — см. [backend.md](backend.md).
...
| **Источники / транспорт / БД** | коннекторы облаков, протокол передачи, SQL | Ktor и SQLDelight |
...
| Данные | схема и запросы SQLDelight | драйверы SQLite |
...
Swift-код на iOS — только точка входа и то, что нельзя сделать из Kotlin/Native.
...
Ограничения iOS не обходим хитростями, а объясняем пользователю: «Откройте Homeroll дома — фото уедут на ПК».
