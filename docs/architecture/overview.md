...
flowchart TB
  ...
  CORE1 -- "поиск в сети + HTTPS, ключи из QR" --> SRV
```
...
sequenceDiagram
  ...
  P->>D: найти друг друга в сети, HTTPS + вход по ключу устройства
...
- **Превью:** Coil 3 с собственными загрузчиками для каждого источника, кэш в памяти и на диске.
  Для галереи — системные превью (PhotoKit, `loadThumbnail` на Android), без скачивания и декодирования оригиналов:
  спайк S-02 показал, что с ними сетка из 11 000 фото плавная на 60 Гц, а с декодированием — нет.
