...
**Issues → Milestones → New milestone.** Вехи названы по этапам строительства дома — Homeroll и есть
«дом» для фотографий. Номер версии пишем в описание вехи.

| Title | Description | Эпики |
|---|---|---|
| **Фундамент** | `v0.1` · ядро, база данных, галерея, единая лента | #1, #2, #3, #4 |
| **Проводка** | `v0.2` · связка телефона и ПК, передача, автоархив | #5 |
| **Крыша** | `v0.3` · облака: Яндекс Диск, Dropbox, OneDrive, WebDAV, Google | #6 |
| **Генеральная уборка** | `v0.4` · дубли и безопасное освобождение места | #7 |
| **Новоселье** | `v1.0` · бета, полировка и первый релиз в сторах | #8 |
