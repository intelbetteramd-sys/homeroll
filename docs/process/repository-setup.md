...
Вехи создаются **вручную**: **Issues → Milestones → New milestone**, по таблице ниже.
Название должно начинаться с номера версии — по нему эпики привязываются к вехе.

Привязка эпиков — автоматически. После синхронизации лейблов workflow
[`Roadmap setup`](../../.github/workflows/roadmap-setup.yml) вешает на эпики #1–#8 лейблы и кладёт каждый эпик
в веху, название которой начинается с его версии. Он ничего не создаёт и не удаляет, лейблы только добавляет,
а эпик с уже назначенной вехой не переносит.

- [ ] Проверить: **Issues → Milestones** — в каждой вехе свои эпики.
