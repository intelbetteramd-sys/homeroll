...
class JankMeter(window: Window) {
    ...
    private val durations = FloatArray(SpikeConstants.FRAME_WINDOW)
    ...
    private fun record(frame: FrameData) {
        val millis = frame.frameDurationUiNanos.toFloat() / SpikeConstants.NANOS_PER_MILLI
        synchronized(lock) {
    ...
    fun snapshot(): FrameStats = synchronized(lock) {
        ...
        FrameStats(
            frames = frames,
            janky = janky,
            p50Millis = percentile(SpikeConstants.P50),
            p90Millis = percentile(SpikeConstants.P90),
            p99Millis = percentile(SpikeConstants.P99),
            maxMillis = maxMillis,
        )
    }
}
